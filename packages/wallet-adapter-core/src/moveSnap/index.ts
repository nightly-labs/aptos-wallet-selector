...
export class NightlySnap implements Wallet {
  ...
  private _invokeSnap:
    | (({ method, params }: InvokeSnapParams) => Promise<unknown>)
    | null = null;
  ...
  get isMetamaskReady() {
    return isLocalSnap(defaultSnapOrigin)
      ? this._isFlaskInstalled
      : this._provider !== null;
  }
  ...
  public signAndSubmitTransaction: AptosSignAndSubmitTransactionMethod = async (
    input
  ) => {
    try {
      if (!this._invokeSnap || !this._provider) {
      ...
      const aptosConfig = new AptosConfig({
        fullnode:
          this.networkInfo.url ?? NETWORK_MAP[this.networkInfo.chainId]?.url,
  ...
  changeNetwork: AptosChangeNetworkMethod = async (input) => {
    try {
      ...
      if (
        this.networkInfo.name === input.name &&
        this.networkInfo.chainId === input.chainId
      ) {
        return {
          status: UserResponseStatus.APPROVED,
          args: {
